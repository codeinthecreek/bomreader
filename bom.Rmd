...
With the BoM data loaded and preprocessed it can now be analysed.

## Find mean temperature for location over period covered by data
...
```{r}
# find hottest time of day, and it's avg temp, per location
# note that time intervals are half hourly, so don't need to bin times
# TODO there should be a simpler way of doing this
hottest_time_loc <- temp_obs %>% 
  select(name, wtime, wdate, air_temp) %>% # relevant fields only
  group_by(name, wdate) %>% # for each location and day
  filter(air_temp==max(air_temp)) %>% # keep maximum temp entry
  group_by(name, wtime) %>% # group by time and then count days
  summarise(avg_max=mean(air_temp), freq=n()) %>% # it was max, and avg
  group_by(name) %>% # then per location
  top_n(1, freq) %>%  # only keep the time with the highest count 
  group_by(name, freq) %>% # if many times have highest count
  filter(avg_max==max(avg_max)) # break ties with higher temp

# summarise(hottest_hr=whr[which.max(n)], freq_of_max=max(n), avg_max=mean(air_temp))

coldest_time_loc <- temp_obs %>% 
  select(name, wtime, wdate, air_temp) %>%
  group_by(name, wdate) %>%
  filter(air_temp==min(air_temp)) %>%
  group_by(name, wtime) %>%
  summarise(avg_min=mean(air_temp), freq=n()) %>%
  group_by(name) %>%
  top_n(1, freq) %>%
  group_by(name, freq) %>%
